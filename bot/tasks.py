...
def pl(n, one, few, many):
    """Слово в нужной форме: 1 ряд, 3 ряда, 5 рядов."""
    n = abs(n) % 100
    if 11 <= n <= 14:
        return many
    n %= 10
...
def make(grade):
    grade = grade or 3
    spring = _school_month() in (1, 2, 3, 4, 5)
    if grade <= 1:
        # 1 класс: до Нового года в пределах 10, после с переходом через десяток до 20
        return random.choice([_add10, _sub10, _story10, _missing10] + ([_add20, _sub20] if spring else []))()
    if grade == 2:
        # умножение во 2 классе начинают во второй половине года
        late = _school_month() in (2, 3, 4, 5)
        return random.choice([_add, _sub] + ([_mult_small, _rows] if late else []))()
    if grade <= 4:
...
def _add10():
    ...
    return {"q": f"Посчитай: {a} + {b}", "a": a + b,
            "hints": ["Начни с большего числа и досчитай.",
                      f"Держи в уме {big} и прибавляй по одному.",
                      f"{big}, а дальше ещё {small} {pl(small, 'шаг', 'шага', 'шагов')} вперёд."],
            "steps": f"{big} и ещё {small}, получается {a + b}"}
...
def _sub10():
    ...
    return {"q": f"Посчитай: {a} − {b}", "a": a - b,
            "hints": ["Когда вычитаем, часть убираем.",
                      f"Начни с {a} и отсчитай назад {b} {pl(b, 'шаг', 'шага', 'шагов')}.",
                      f"Можно проверить сложением: сколько надо прибавить к {b}, чтобы получилось {a}?"],
            "steps": f"от {a} назад на {b}, получается {a - b}"}


def _story10():
    a = random.randint(2, 6)
    b = random.randint(1, 10 - a)
    name = random.choice(["У Маши", "У Пети", "У Оли", "У Димы"])
    return {"q": f"{name} было {a} {pl(a, 'яблоко', 'яблока', 'яблок')}. Мама дала ещё {b}. Сколько яблок стало?",
            "a": a + b,
            "hints": ["Яблок стало больше или меньше?",
                      "Стало больше, значит, нужно сложить.",
                      f"{a} + {b}."],
            "steps": f"{a} + {b} = {a + b}"}
...
def _missing10():
    ...
    return {"q": f"Какое число нужно прибавить к {a}, чтобы получилось {c}?", "a": c - a,
            "hints": [f"Вспомни, из каких чисел состоит {c}.",
                      f"{a} и сколько ещё будет {c}?",
                      f"Посчитай от {a} до {c}."],
            "steps": f"{a} + {c - a} = {c}, значит, нужно прибавить {c - a}"}
...
def _add20():
    ...
    return {"q": f"Посчитай: {a} + {b}", "a": a + b,
            "hints": ["Сначала дополни до десяти.",
                      f"{a} + {10 - a} = 10.",
                      f"Осталось прибавить к 10 ещё {b - (10 - a)}."],
            "steps": f"{a} + {10 - a} = 10, 10 + {b - (10 - a)} = {a + b}"}
...
def _sub20():
    ...
    return {"q": f"Посчитай: {a} − {b}", "a": a - b,
            "hints": ["Сначала вычти столько, чтобы получилось 10.",
                      f"{a} − {a - 10} = 10.",
                      f"Осталось вычесть из 10 ещё {b - (a - 10)}."],
            "steps": f"{a} − {a - 10} = 10, 10 − {b - (a - 10)} = {a - b}"}
...
def _mult_small():
    ...
    return {"q": f"Посчитай: {a} · {b}", "a": a * b,
            ...
            "steps": f"{a} · {b} означает: {b} сложить {a} раза, получится {a * b}"}


def _rows():
    r, c = random.randint(3, 7), random.randint(3, 6)
    return {"q": f"В коробке {r} {pl(r, 'ряд', 'ряда', 'рядов')}, в каждом по {c} {pl(c, 'конфете', 'конфеты', 'конфет')}. Сколько конфет всего?", "a": r * c,
            "hints": ["Рядов несколько, в каждом поровну. Это умножение.",
                      f"Посчитай {r} раз по {c}.",
                      f"{r} · {c} — вспомни таблицу."],
            "steps": f"{r} {pl(r, 'ряд', 'ряда', 'рядов')} по {c} {pl(c, 'конфете', 'конфеты', 'конфет')}, значит, {r} · {c} = {r * c}"}
...
def _order():
    b, c = random.randint(3, 9), random.randint(4, 11)
    a = b * c + random.randint(4, 50)          # ответ всегда положительный: минус в 3 классе не проходят
...
def _price():
    ...
    return {"q": f"Тетрадь стоит {price} рублей. Сколько стоят {n} {pl(n, 'тетрадь', 'тетради', 'тетрадей')}?", "a": price * n,
...
def _order5():
    b, c, d = random.randint(3, 8), random.randint(9, 18), random.randint(2, 9)
    a = b * (c + d) + random.randint(5, 60)
...
def _percent():
    ...
    return {"q": f"Найдите {p} {pl(p, 'процент', 'процента', 'процентов')} от {whole}.", "a": whole * p // 100,
            "hints": ["Один процент равен сотой части числа.",
...
def _speed():
    ...
    return {"q": f"Велосипедист едет со скоростью {v} км/ч. Какой путь он проедет за {t} {pl(t, 'час', 'часа', 'часов')}?",
            "a": v * t,
            "hints": ["Путь равен скорости, умноженной на время.",
                      f"За один час {v} км.",
                      f"Значит за {t} {pl(t, 'час', 'часа', 'часов')} {v} · {t}."],
...
def _two_parts():
    ...
    return {"q": f"Два числа в сумме дают {total}, причём второе в {k} раза больше первого. "
            ...
            "hints": ["Обозначьте меньшее число за одну часть.",
                      f"Тогда большее число составляет {k} части, а вместе {k + 1} {pl(k + 1, 'часть', 'части', 'частей')}.",
...
def _raise():
    ...
    return {"q": f"Товар стоил {w} рублей и подорожал на {p} {pl(p, 'процент', 'процента', 'процентов')}. Какой стала цена?",
            "a": w + w * p // 100,
...
def _mean():
    ...
    return {"q": f"Найдите среднее арифметическое чисел: {', '.join(map(str, nums))}.", "a": m,
            "hints": ["Среднее арифметическое равно сумме всех чисел, делённой на их количество.",
...
def _quadratic():
    ...
    if r1 == r2:
        r2 = 5 if r1 == 6 else r1 + 1
    b, c = -(r1 + r2), r1 * r2
    sb = "" if b == 0 else (f" + {'' if b == 1 else b}x" if b > 0 else f" − {'' if b == -1 else -b}x")
    sc = "" if c == 0 else (f" + {c}" if c > 0 else f" − {-c}")
    lo, hi = min(r1, r2), max(r1, r2)
    return {"q": f"Решите уравнение и введите больший корень: x²{sb}{sc} = 0", "a": hi,
            "hints": ["Теорема Виета: сумма корней равна второму коэффициенту с обратным знаком, "
                      ...
                      f"Подберите два целых числа с такой суммой и произведением. Меньшее из них {lo}."],
            "steps": f"x₁ = {lo}, x₂ = {hi}; больший корень {hi}"}
...
def _discount():
    ...
    return {"q": f"Куртка стоила {w} рублей, на распродаже скидка {p} {pl(p, 'процент', 'процента', 'процентов')}. "
                 f"Сколько рублей стоит куртка со скидкой?", "a": w - w * p // 100,
            "hints": ["Скидка составляет часть цены. Найдите её в рублях.",
                      f"{w} · {p} : 100 = {w * p // 100}.",
