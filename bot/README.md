...
- личный кабинет на сайте: вход из бота одноразовой ссылкой, в Telegram прямо внутри мессенджера;
- поддержка: Мыслик отвечает на вопросы об оплате и возврате, заявки приходят администратору;
- бесплатный доступ навсегда для своих: /free у администратора;
...
| `webhook.py` | сервер сайта: личный кабинет (/api), оплата ЮKassa и Tribute, поддержка |
| `pay_yookassa.py` | создание и проверка платежей ЮKassa: карта, МИР, СБП по QR |
| `support.py` | ответы поддержки из `assets/support.json`, заявки на возврат, `/reply` |
| `ids.py` | номера в общей базе: Telegram как есть, MAX со знаком минус, сайт от 9e18 |
| `db.py` | одна база SQLite на оба бота и сервер, создаётся сама |
...
| `FREE_USERS` | нет | бесплатный доступ навсегда, номера через запятую; удобнее командой /free |
| `MAX_CHAT_ID_CLOSED` | нет | закрытый канал в MAX для утренних заданий |
| `SITE_URL` | нет | адрес сайта, по умолчанию https://smekairu.github.io/Smekai/ |
| `API_PUBLIC` | нет | внешний адрес сервера кабинета с HTTPS; пока пусто, кабинет работает на устройстве |
| `SITE_ORIGINS` | нет | сайты, которым сервер разрешает запросы, по умолчанию smekairu.github.io |
| `YOOKASSA_SHOP_ID`, `YOOKASSA_SECRET` | для оплаты на сайте | ключи магазина ЮKassa |
| `YOOKASSA_RECEIPT` | нет | 1, если подключены «Чеки от ЮKassa» |
| `PAY_URL` | нет | страница оплаты, по умолчанию oplata.html на сайте |
...
Основной путь: оплата на сайте через ЮKassa (карта, МИР, СБП по QR), доступ открывается сам.
Настройка в `docs/oplata.md`. Пока ЮKassa не подключена, страница оплаты ведёт на Boosty,
а доступ открывается вручную: `/grant id дней тариф`, например `/grant 123456789 30 myslik`.

Tribute (оплата внутри Telegram) тоже поддерживается: адрес уведомлений `https://ваш-домен/pay`,
ключ в `PAY_WEBHOOK_SECRET`, тариф по умолчанию `TRIBUTE_PLAN=myslik`.
