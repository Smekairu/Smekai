...
- стикеры с Мысликом на события: приветствие, верно, ошибка, серия из трёх, думает, устал;
...
| `VOICE_BOY`, `VOICE_GIRL`, `VOICE_PARENT`, `VOICE_LEV` | нет | имена голосов Яндекса, по умолчанию anton, masha, alexander, ermil |
...
## Наборы стикеров и эмодзи в Telegram

Скрипт `stickerpack.py` создаёт наборы от имени бота, владельцем становится ADMIN_ID:
Мыслик (11 стикеров), Лев (11), живой Мыслик (10 видеостикеров) и набор эмодзи (22).

```bash
set -a; . ./.env; set +a
python stickerpack.py            # все наборы
python stickerpack.py --only emoji
```

Ссылки на наборы печатаются в конце. Эмодзи в сообщениях доступны подписчикам Premium,
это ограничение Telegram, а не наше. В MAX наборы стикеров загружаются вручную через
каталог стикеров, файлы для этого лежат в `assets/pack/sticker`.

## Выбор голоса

Команда `/voicetest` для администратора присылает одну фразу всеми голосами Яндекса подряд,
с подписью, какой это голос. Понравившиеся вписываются в `.env`: VOICE_BOY, VOICE_GIRL,
VOICE_PARENT, VOICE_LEV. Свою фразу можно передать после команды: `/voicetest Привет, что решаем?`
